...
app.get('/api/tweets', (req, res) => {
    try {
        ...
        // Thread filtering is handled conditionally by excludeThreads checkbox below
...
app.get('/api/broadcast/bluesky/debug', async (req, res) => {
    ...
    log("Starting debug test with session resume...");
    ...
    try {
        ...
        log("Calling api.post()...");
        const postRes = await api.post(text);
...
app.get('/api/swipe/queue', (req, res) => {
    try {
        ...
        const query = `
            ...
            WHERE ${whereClause}
            ORDER BY 
                -- Prioritize Long Tweets (>1000 chars)
                CASE WHEN LENGTH(t.full_text) >= 1000 THEN 0 ELSE 1 END,
                -- Then Threads
                CASE WHEN t.combined_text IS NOT NULL THEN 0 ELSE 1 END,
                -- Then by engagement
